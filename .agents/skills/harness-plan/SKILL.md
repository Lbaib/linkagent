...
3. `.harness/prompts/planner.md` — Sprint 规划模板
...
1. **数 requirement**：从 `openspec/changes/<change>/specs/<openspec_feature>/spec.md` 中 grep `^### Requirement:` 行数，得到 Requirement 数 N。
2. **数 task**：从 `openspec/changes/<change>/tasks.md` 中 grep `^- \[[ x]\] [0-9]+\.[0-9]+` 行数，得到 task 数 M（tasks.md 缺失则跳过此项）。
3. **判定**：
    - N > 3 → **BLOCK** · 输出："Spec 含 N 个 requirement，超过原则 XII 阈值 3。请先拆分为 N 个独立 change，再依次跑 opsx propose → design → tasks → harness.plan。"
    - M > 30 → **BLOCK** · 输出："Tasks.md 含 M 个 task，超过原则 XII 阈值 30。这是 Sprint 49 类型的爆炸前兆，必须先拆 spec 再重新生成 tasks。"
    - 两者都 ≤ 阈值 → 继续 Step 1。
4. **3 选 1 处理建议**（BLOCK 时一并输出）：
    - 拆 spec（推荐）：列出当前 N 个 requirement 标题，建议按 requirement 边界切成 N 个 change
    - 合并 requirement：仅当几个 requirement 真正紧密耦合（独立交付不构成价值）
...
如需手工跑独立审计：`/harness.spec-check <change>`。
...
3. 读取 planner.md，理解规划规则和工时估算基准
4. 按规则生成 Sprint 计划，每个 Sprint 包含：
    - 目标和对应 Phase
    - 按 Day/Batch 组织的任务清单（含 Task UID、并行标记和预估工时）
    - 验证检查点
    - 风险项
5. 将 Sprint 计划写入 `.harness/sprints/<feature>/sprint-{n}.md`
...
7. **👁 人工验证节点 HV 强制识别**（Sprint 47 血教训沉淀 · 2026-04-24）:
...
8. 输出 Sprint 总览表 · 表中含 `ticket_id`、`openspec_feature`、`feature`、`HV 节点数` 列
