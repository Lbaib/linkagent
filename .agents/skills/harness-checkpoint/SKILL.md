...
读取项目根 `AGENTS.md` 的 `custom_evaluator`；若存在，先执行 `/harness.eval custom-deferred`（或等价：按该文件与 `.harness/prompts/evaluator.md` 同步骤对照，处理 `sprint-{N}-custom-eval-deferred.md`）。未清账不得进入 Constitution 评分。安全类未决 FAIL → 先 Corrector；非安全类 FAIL → 记入 checkpoint 扣分或要求修复后再 PASS。
...
3. 读取 `.harness/sprints/<feature>/sprint-{N}-progress.md`，了解本 Sprint 完成的任务和文件变更
4. 逐条审查：
...
### 原则 II: 接口契约驱动
- 读取 `openspec/changes/<change>/specs/<openspec_feature>/` 相关契约或 `[API_CONTRACT_ROOT]` 中所有已实现的接口契约
