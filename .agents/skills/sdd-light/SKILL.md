...
name: sdd.light
description: >-
  编排轻量 SDD 交付流水线：解析需求（正文或 IPD 附件）后依次执行
  opsx propose → harness.plan.light → harness.scope.light → harness.start → harness.exec(batch)
  → harness.eval.light → harness.checkpoint.light → harness.metrics.light → opsx archive。
  每个环节结束后必须等人确认再进入下一步；不确认则按用户修改意见回退重做。
  Use when the user runs sdd.light / sdd-light / 轻量交付, or wants the light Harness
  variants of the OpenSpec+Harness pipeline.
triggers:
  - sdd.light
  - sdd light
  - sdd-light
...
按固定顺序编排 OpenSpec + **Harness light** 交付链路。**每个环节结束后必须等待人为确认**，
...
| 环节 | `sdd.full` | `sdd.light` |
|------|------------|-------------|
| plan | `/harness.plan` | `/harness.plan.light` |
| scope | `/harness.scope` | `/harness.scope.light` |
| start / exec | 完整版 | **同完整版**（无 light 变体） |
| metrics | `/harness.metrics` | `/harness.metrics.light` |
| eval（含 custom-deferred） | `/harness.eval` | `/harness.eval.light` |
| checkpoint | `/harness.checkpoint` | `/harness.checkpoint.light` |
| 画像台账文件 | `sdd-full-profile-usage.jsonl` | `sdd-light-profile-usage.jsonl` |
...
```
/sdd.light dpahcm-1416 支持 PCDN 域名鉴权并回写属性
...
## Preflight · `full_rule.md` 强制升级检查（阻断式）

在解析需求、进入任何编排环节**之前**，必须先做本检查：

1. 读取项目根目录 `full_rule.md`（相对业务项目根，与 `AGENTS.md` 同级）
2. **文件不存在** → 跳过本检查，继续 Step 0
3. **文件存在** → 读取全文规则描述，结合本轮已有上下文判断该 change 是否应走完整流水线：
   - `change_name`
   - `$ARGUMENTS` 中的需求描述（若有）
   - 若尚无需求正文、但已具备 IPD 摘要/附件标题等，可一并纳入判断（不得为做判断而提前跑 propose / plan.light）
4. **判定命中「应使用 sdd.full」** → **立即中断** `/sdd.light`，**禁止**进入 propose / plan.light / scope.light 及后续任何环节；向用户输出：
   - 命中的规则要点（引用 `full_rule.md` 中的对应描述，勿编造）
   - 强制改用完整流水线，例如：
     ```text
     ⛔ 本 change 命中项目根 full_rule.md，必须使用 sdd.full，已中断 sdd.light。
     请改执行：/sdd.full <change_name> [原需求描述或 --user/--pass …]
     ```
5. **未命中** → 继续 Step 0

`full_rule.md` 由项目自行维护（规则文案、匹配条件以文件内容为准）。本 skill **只负责按文件内规则诚实判定**；无法判断时向用户展示规则原文并询问，**不得默认放行 light**（有歧义时倾向中断并建议 `/sdd.full`）。
...
| 2 | plan | 执行 `/harness.plan.light`（harness-plan-light skill），绑定本 change |
| 3 | scope | 执行 `/harness.scope.light`（harness-scope-light skill） |
| 4 | start | 执行 `/harness.start`（harness-start skill；无 light） |
| 5 | exec | 执行 `/harness.exec`（harness-exec skill；无 light），按 **batch** 推进；可多次，直到本 Sprint 全部 batch / 门禁完成 |
| 5b | custom-eval | （可选）若 `AGENTS.md` 配置了 `custom_evaluator`：执行 `/harness.eval.light custom-deferred`（harness-eval-light skill），清账延期补充门禁；**须在 checkpoint 之前完成**。未配置时，仍建议在进 checkpoint 前跑一次 `/harness.eval.light`（全批次单次检查） |
| 6 | checkpoint | 执行 `/harness.checkpoint.light`（harness-checkpoint-light skill） |
| 7 | metrics | 执行 `/harness.metrics.light`（harness-metrics-light skill） |
...
未配置 `custom_evaluator` 时：跳过「仅 custom-deferred」语义，但 **5b 仍应用 `/harness.eval.light` 做全批次收官检查**（与 light eval 默认行为一致），再进 checkpoint。确认门禁序号按实际执行环节计数。
...
```text
.harness/metrics/<feature>/sdd-light-profile-usage.jsonl
```
...
6. `metrics` 环节自身也记一条；再在 `sprint-{N}-light.json` 中**汇总**台账（见下）
...
执行 `/harness.metrics.light` 时：
1. 按 light 规则仅根据本轮运行状态写 `.harness/metrics/<feature>/sprint-{N}-light.json`（**不做**五维公式与趋势）
2. 读取 `.harness/metrics/<feature>/sdd-light-profile-usage.jsonl`（本流水线）；若同时存在 `sdd-full-profile-usage.jsonl` 勿混用，以本轮 `pipeline: sdd.light` 台账为准
3. 将画像汇总写入 light 指标文件的 `projectProfileUsage`（**不含** `clarifyCount`）。对象**只允许**下列字段，格式固定：

```json
"projectProfileUsage": {
  "profileDir": "/path/to/",
  "stepsConsulted": ["propose", "exec"],
  "stepsNotConsulted": ["plan", "scope", "start", "eval.light", "checkpoint.light", "metrics.light", "archive"],
  "profileFilesUsed": [
    "context/overview.md",
...
light 全量环节清单（写入 `stepsConsulted` / `stepsNotConsulted` 时用这些名字，**不要**写 jsonl 里的 `custom-eval`）：

`propose`, `plan`, `scope`, `start`, `exec`, `eval.light`, `checkpoint.light`, `metrics.light`, `archive`

- jsonl `step: "custom-eval"` → 汇总时写成 `eval.light`
- jsonl `step: "checkpoint"` / `"metrics"` → 汇总时写成 `checkpoint.light` / `metrics.light`
- 未配置 `custom_evaluator` 时仍保留 `eval.light`（对应编排 5b 全批次检查）
- **禁止**把 `clarifyCount` 放进 `projectProfileUsage`（澄清次数只写报告顶层）

4. 将各行 `clarifyCount` **求和**写入同一文件顶层字段 `clarifyCount`
5. 读取 `.harness/metrics/<feature>/sdd-light-skill-tokens.jsonl`，汇总写入报告 顶层 `inputTokens` / `outputTokens` / `tokens` 与 `tokenUsage`（**不要**写 `skillTokenUsage`；见「Skill Token 消耗台账」）
6. 终端只打短摘要（含顶层 `clarifyCount`、顶层 `inputTokens` / `outputTokens` / `tokens` / `tokenUsage`）

若 profile jsonl 缺失 → `projectProfileUsage` 写明缺口（可加 `"missingLedger": true`），`stepsConsulted` / `profileFilesUsed` / `keySections` 用 `[]`，`stepsNotConsulted` 填 light 全量清单；顶层 `clarifyCount: null`，并加入 `dataGaps`，不得伪造分步数据。
若 token jsonl 缺失 → `inputTokens` / `outputTokens` / `tokens` 为 `null`、`tokenUsage: []`，并加入 `dataGaps`，不得伪造 token。

## Skill Token 消耗台账（强制）

与画像台账并列：对流水线**每个环节**执行的 skill 计算 token 消耗，写入 metrics 目录中间文件；**metrics 环节**再汇总进 `sprint-{N}-light.json`。

在 `sdd.light` 编排内，token 台账以 metrics 目录中间文件与 metrics 产出为准。
...
```text
.harness/metrics/<feature>/sdd-light-skill-tokens.jsonl
```
...
5. `metrics` 环节自身也记一行（含读台账与写 `sprint-*-light.json` 的消耗）
6. 确认摘要增加：`Token: model=<modelName> · in=<n> · out=<m> · total=<t> · est=<approx|reported> · 已写入 sdd-light-skill-tokens.jsonl`

### metrics 汇总字段（顶层 `inputTokens` / `outputTokens` / `tokens` / `tokenUsage`）

写入 `.harness/metrics/<feature>/sprint-{N}-light.json` 顶层（**不要**包在 `skillTokenUsage` 内）：
...
- 使用 `/harness.exec`（无 light 变体），按批次执行
- 一批结束后若仍有未完成 batch → 继续下一 batch；**不必**每批跟跑 `/harness.eval`（与 light 策略一致：收官再验）
- 全部 batch 跑完（或无法继续）→ 将该「exec」环节视为完成，进入人工确认
- 中途主门禁 FAIL 且用户要求修正 → 可在本环节内走 `/harness.fix` 后继续 batch；仍算同一 exec 环节
- **自定义补充门禁**：exec 中仅主门禁可打断；非安全补充记 deferred。确认后先跑环节 5b（`/harness.eval.light`，含 custom-deferred / 全批次检查），再进 `/harness.checkpoint.light`
...
画像使用: consulted=<yes|no> · files=<n> · sections=<m> · clarifications=<k> · profileDir=<path> · feature=<feature> · 已写入 sdd-light-profile-usage.jsonl
Token: model=<modelName> · in=<n> · out=<m> · total=<t> · est=<approx|reported> · 已写入 sdd-light-skill-tokens.jsonl
...
3. 项目画像使用汇总（咨询率、缺口；指向 `sdd-light-profile-usage.jsonl` 与 `sprint-{N}-light.json` 中的 `projectProfileUsage`）
4. 澄清修改次数合计（报告顶层 `clarifyCount`）
5. Skill Token 汇总（指向 `sdd-light-skill-tokens.jsonl` 与 `sprint-{N}-light.json` 顶层 `inputTokens` / `outputTokens` / `tokens` / `tokenUsage`）
6. 关键产物路径：OpenSpec change、sprint plan/progress、scope、checkpoint、`.harness/metrics/<feature>/sprint-{N}-light.json`、archive
7. 若中途中止：停在哪一环节、待办建议

## 示例

```
/sdd.light dpahcm-1416 支持 PCDN 域名鉴权
/sdd.light dpahcm-1416 --user alice --pass '***'
```
