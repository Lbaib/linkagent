...
**补充门禁时机**：
- **批次中 / 紧跟 exec**：主门禁失败仍阻断；非安全类补充 FAIL → 只写入 `sprint-{N}-custom-eval-deferred.md`，**不**要求当场中断修复
- **安全类补充 FAIL**：与主门禁同等，可进 Corrector
- **全批次完成、checkpoint 前**：必须跑完补充门禁（含 deferred 清账）；可用 `/harness.eval custom-deferred` 或由 `/harness.checkpoint` 前置步骤触发

### 输入参数

$ARGUMENTS — 验证级别和范围，格式: `{level} {任务ID、Task UID或范围}`

- `/harness.eval L2 DPAHCM-1001-do-someting-1.1` — 对任务 DPAHCM-1001-do-someting-1.1 执行 Level 2 契约验证
- `/harness.eval L2 1.1` — 对当前 change 的任务 1.1 执行 Level 2 契约验证
- `/harness.eval L4` — 执行 Constitution 全面合规审查
- `/harness.eval all` — 执行全部四级验证（Sprint Checkpoint 时使用）
- `/harness.eval custom-deferred` — 仅跑 AGENTS.md `custom_evaluator` 补充门禁（含 deferred 清账）；用于全批次后、checkpoint 前（`sdd.full` 环节 5b）
- 不带参数 — 默认对最近完成的批次执行 **L1 + L2**（高风险任务自动加 L4）；补充非安全项仍按延期策略
