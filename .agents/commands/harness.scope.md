...
**上下文管理**: 保持当前上下文（需要 CLAUDE.md / spec / design 多处对照）
...
为当前 Sprint 生成 `.harness/scope/<feature>/sprint-<N>.yaml`，作为 boundary-reviewer 的输入和 evaluator L2.4 的对照标的。

读取以下文件：

1. `.harness/scope/<feature>/_template.yaml` — scope schema 模板
2. `CLAUDE.md`（项目根）— 已有的"焦点 / 出范围"叙述（如存在）
3. `.harness/sprints/<feature>/sprint-<N>.md` — 当前 Sprint 计划
4. `openspec/changes/<change>/design.md` — 本 change 的 Project Structure 段（如存在）
...
1. 优先读取 `openspec/changes/<change>/design.md` 的 Project Structure / Source Code 段
...
**优先级 B · spec + design 推断（次优）**

如果 CLAUDE.md 没有这张表：

- 读 `openspec/changes/<change>/design.md` 的 Project Structure / Source Code 段，列出本 change 涉及的模块 → `in_scope`
- 扫 `[SOURCE_ROOT]` 下的所有顶级模块，未列入 in_scope 的默认入 `out_of_scope`，`reason: "本 change design 未涉及"`
...
4. 对每个 out_of_scope 模块问一句 reason（可批量复用："非当前 change 范围" 等）
...
把 Step 4 用户确认后的 yaml 草案写到**临时位置** `.harness/scope/<feature>/sprint-<N>.draft.yaml`（不是最终路径），并加 discovery flag：

```yaml
metadata:
  discovery_mode: true
  report_path: .harness/scope/<feature>/sprint-<N>.discovery-report.md
known_violations: []
```
...
- 参数：`scope_yaml_path = .harness/scope/<feature>/sprint-<N>.draft.yaml`
...
写回 `.harness/scope/<feature>/sprint-<N>.draft.yaml`。
...
- 提示用户在 sprint-N.md / `openspec/changes/<change>/tasks.md` 加一条解耦 task

upgrade-scope 选项需要：

- 把 to 模块从 out_of_scope 移到 in_scope
- 同步提示用户：CLAUDE.md / spec / design 可能需要回填修订

#### Step 7 · 写盘 + 摘要

写入 `.harness/scope/<feature>/sprint-<N>.yaml`。

输出摘要给用户：

```
✅ .harness/scope/<feature>/sprint-<N>.yaml 已生成
  ...
  known_violations:
    ...
    upgrade-scope: <count>（已调整 in_scope · 请检查 spec/design）
...
- **CLAUDE.md / spec / design 与 yaml 的一致性**由下次 boundary-reviewer 在 L2.4 跑时检查（`consistency_warnings`），本命令不强制对齐这三处。
