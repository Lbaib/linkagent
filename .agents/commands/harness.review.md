...
3. `openspec/changes/<change>/specs/<openspec_feature>/spec.md` — 当前 change spec（自动从 task ID 推断或用户传参）
4. git diff（默认 `HEAD~1..HEAD`，可参数指定 base）

### 输入参数

- Task ID / Task UID：`$ARGUMENTS`（必传 · 例如 `1.1`、`DPAHCM-1001-do-someting-1.1` 或 `do-someting`）
...
- 从 OpenSpec change name 推导 metadata，确认 `ticket_id` / `openspec_feature` / `feature` / `task_uid_prefix`
- 如果参数是完整 Task UID 形式（如 `DPAHCM-1001-do-someting-1.1`，即 `<task_uid_prefix>-<openspec_feature>-<task_id>`），按末尾 `-<task_id>` 切分；如果参数是裸 `1.1`，只在当前 change 内解析
- 从 task ID 反查 spec：扫 `openspec/changes/<change>/tasks.md`，找到包含该 task ID 的条目
...
- 参数：`task_uid` + `task_id` + `ticket_id` + `base_commit` + diff 内容

Sub-agent 按其 Step 0-5 走完，写盘 `.harness/reviews/<feature>/<task-id>.md`，输出 `PEER_REVIEWER_RESULT`。
...
报告: .harness/reviews/<feature>/<task-id>.md
...
完整报告打开 .harness/reviews/<feature>/<task-id>.md
```

### 注意

- **只读命令**：不修改任何代码、不修改 spec、不修改 task 状态。仅产出 review 报告。
- **报告存档**：`.harness/reviews/<feature>/<task-id>.md` 持久化保存。这一文件是工程师未来排查问题的入口——尤其那段"排查指引"是 AI 写完代码 N 个月后的导航地图。
