...
遵循 `harness-openspec` skill：任务完成状态同步规则见 § 任务完成状态同步。
...
5. **更新进度文件与 OpenSpec tasks.md**:
    - 记录修正详情到修正记录表
    - 更新任务验证状态（progress 标记 `[x]` 时，同步 `openspec/changes/<change>/tasks.md` 对应 `task_id` 行，规则见 `harness-openspec` § 任务完成状态同步）
