...
输出：
- 写盘到 `.harness/reviews/[FEATURE_ID]/<task-id>.md`（持久化 · 工程师以后查 task 历史时能读到）
...
按以下模板写到 `.harness/reviews/[FEATURE_ID]/<task-id>.md`：
...
- **持久化**: report 写到 `.harness/reviews/[FEATURE_ID]/<task-id>.md` · 工程师未来查 task 历史时能直接读到 · 不会丢失"排查指引"那段知识
