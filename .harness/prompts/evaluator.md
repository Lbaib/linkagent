...
**触发条件**：本 Sprint 存在 `.harness/scope/[FEATURE_ID]/sprint-<N>.yaml`。

**执行**：调用 boundary-reviewer sub-agent（`harness/prompts/boundary-reviewer.md`）。
Sub-agent 会：
1. 读 `.harness/scope/[FEATURE_ID]/sprint-<N>.yaml`
...
3. 写盘 `.harness/reviews/[FEATURE_ID]/<task-id>.md`（持久化心智模型，工程师以后能查）
...
PEER_REVIEWER_RESULT:
{
  ...
  "suggestion": <count>,
  "sensor_gap": <count>,
  "report_path": ".harness/reviews/[FEATURE_ID]/<task-id>.md"
}
```
...
**双观众输出的存档价值**：每个 task 一份 `.harness/reviews/[FEATURE_ID]/<task-id>.md`。"排查指引"那段是工程师未来排查问题的入口——AI 写完代码 N 个月后 bug 出现时，工程师不用重新读代码，先读这份 review。
