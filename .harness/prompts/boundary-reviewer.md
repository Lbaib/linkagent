...
输入：
- `.harness/scope/[FEATURE_ID]/sprint-<N>.yaml`（必须存在，缺失即 FAIL · 关联 Constitution 原则 XI）
- 项目源码（用于构建模块依赖图）

输出：
- 结构化违规报告（写入 `scope.yaml` 的 `metadata.report_path` 指定路径，默认 `.harness/sprints/[FEATURE_ID]/sprint-<N>-boundary-report.md`）
...
`.harness/scope/[FEATURE_ID]/sprint-<N>.yaml` 申报一致。**不要修改代码**，只输出报告。
